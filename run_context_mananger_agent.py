...
class FeederAreaContextManager(FeederAgent):
    ...
    def __init__(self, upstream_message_bus_def: MessageBusDefinition, 
        ...
        if feeder_dict is None:
            ...
            feeder_dict = self.ot_connection.get_response(REQUEST_FIELD_CONTEXT, request, timeout=10)['data']
    ...
    def on_request(self, message_bus, headers: Dict, message):
        ...
        if message['request_type'] == 'get_context':
            ...
            response = self.ot_connection.get_response(REQUEST_FIELD_CONTEXT, message)
            message_bus.send(reply_to,response)
...
class SwitchAreaContextManager(SwitchAreaAgent):
    ...
    def __init__(self, upstream_message_bus_def: MessageBusDefinition, 
        ...
        if switch_area_dict is None:
            ...
            switch_area_dict = self.ot_connection.get_response(REQUEST_FIELD_CONTEXT, request, timeout=10)['data']
...
class SecondaryAreaContextManager(SecondaryAreaAgent):
    ...
    def __init__(self, upstream_message_bus_def: MessageBusDefinition, 
        ...
        if secondary_area_dict is None:
            request = {'areaId': downstream_message_bus_def.id}
            secondary_area_dict =  self.ot_connection.get_response(REQUEST_FIELD_CONTEXT, request, timeout=10)['data']
...
def _main():
    ...
    for sw_index, switch_area in enumerate(feeder_agent.agent_area_dict['switch_areas']):
        switch_area_message_bus_def = MessageBusDefinition.load(f"config_files_simulated/switch_area_message_bus_{sw_index}.yml")
        ...
        for sec_index, secondary_area in enumerate(switch_area['secondary_areas']):
            secondary_area_message_bus_def = MessageBusDefinition.load(f"config_files_simulated/secondary_area_message_bus_{sw_index}_{sec_index}.yml")
